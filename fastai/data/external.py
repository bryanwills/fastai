...
# %% auto #0
__all__ = ['fastai_cfg', 'fastai_path', 'URLs', 'untar_data', 'nb_urls', 'prefetch_data']
...
# %% ../../nbs/04_data.external.ipynb #c270b46e
def nb_urls(
    cells, # Notebook cells
):
    "Names of the `URLs` datasets referenced in the code of `cells`"
    srcs = [c.source for c in cells if c.cell_type=='code']
    return sorted({m for s in srcs for m in re.findall(r'URLs\.([A-Z_0-9]+)', s)})


# %% ../../nbs/04_data.external.ipynb #33c2dbbf
def prefetch_data(
    cells, # Notebook cells about to be tested
):
    "Download the archive of every dataset `cells` refer to, so parallel tests find it in place"
    fd = FastDownload(fastai_cfg(), module=fastai.data, base='~/.fastai')
    for n in nb_urls(cells):
        url = getattr(URLs, n, None)
        if not isinstance(url, str) or not url.endswith(('.tgz', '.zip')): continue
        if not urldest(url, fd.arch_path()).exists(): print(f'Downloading {n}')
        fd.download(url)
