    ...
    def after_create(self):
        ...
        if self.log_model:
            if not hasattr(self, 'save_model'):
                ...
                if self.model_name is not None: self.save_model.fname = self.model_name
            

# %% ../../nbs/70_callback.wandb.ipynb #6a5da6cb
@patch
def before_fit(self:WandbCallback):
    "Call watch method to log model topology, gradients & weights"
    # Check if wandb.init has been called
    if wandb.run is None: raise ValueError('You must call wandb.init() before WandbCallback()')
    # W&B log step
    ...
    try: wandb.config.update(log_config, allow_val_change=True)
    except Exception as e: print(f'WandbCallback could not log config parameters -> {e}')

    if not WandbCallback._wandb_watch_called:
        WandbCallback._wandb_watch_called = True
        # Logs model topology and optionally gradients and weights
        if self.log is not None: wandb.watch(self.learn.model, log=self.log)
    ...
    if self.log_dataset is True:
        ...
        else: self.log_dataset = self.dls.path
    if self.log_dataset:
...
# %% ../../nbs/70_callback.wandb.ipynb #bd1e6552
@patch
def before_batch(self:WandbCallback): self.ti_batch = time.perf_counter()
...
# %% ../../nbs/70_callback.wandb.ipynb #5b3da0cc
@patch
...
# %% ../../nbs/70_callback.wandb.ipynb #a205d8af
@patch
...
# %% ../../nbs/70_callback.wandb.ipynb #29e45900
@patch
def after_fit(self:WandbCallback):
    if self.log_preds and not self.log_preds_every_epoch: self.log_predictions()
    if self.log_model:
        if self.save_model.last_saved_path is None: print('WandbCallback could not retrieve a model to upload')
        else:
...
def _format_config(config):
    ...
    for k,v in config.items():
        if isinstance(v, dict): config[k] = _format_config(v)
        else: config[k] = _format_config_value(v)
    return config
...
def log_dataset(path, name=None, metadata={}, description='raw dataset'):
    ...
    if wandb.run is None: raise ValueError('You must call wandb.init() before log_dataset()')
    path = Path(path)
    if not path.is_dir(): raise ValueError(f'path must be a valid directory: {path}')
...
def log_model(path, name=None, metadata={}, description='trained model'):
    "Log model file"
    if wandb.run is None: raise ValueError('You must call wandb.init() before log_model()')
    path = Path(path)
    if not path.is_file(): raise ValueError(f'path must be a valid file: {path}')
    ...
    with artifact_model.new_file(str(Path(name).with_suffix(".pth")), mode='wb') as fa: fa.write(path.read_bytes())
...
def wandb_process(x:TensorImage, y, samples, outs, preds):
    ...
    for s,o in zip(samples, outs):
        ...
        for t, capt, res in ((o[0], "Prediction", res_pred), (s[1], "Ground_Truth", res_label)):
            ...
            plt.close(fig)
    return dict(Inputs=res_input, Predictions=res_pred, Ground_Truth=res_label)
...
def wandb_process(x:TensorImage, y:TensorCategory|TensorMultiCategory, samples, outs, preds):
    ...
    for (image, label), pred_label in zip(samples,outs): table.add_data(wandb.Image(image.permute(1,2,0)), label, _unlist(pred_label))
...
def wandb_process(x:TensorImage, y:TensorMask, samples, outs, preds):
    ...
    if codes is not None: class_labels = [{'name': name, 'id': id}  for id, name in enumerate(codes)]
    else: class_labels = [{'name': i, 'id': i} for i in range(preds.shape[1])]
    table = wandb.Table(columns=["Input Image", "Ground_Truth", "Predictions"])
